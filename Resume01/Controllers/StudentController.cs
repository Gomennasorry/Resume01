...
namespace Resume01.Controllers
{
    ...
    public class StudentController : BaseController
    {
        ...
        public StudentController(IHttpClientFactory clientFactory, IConfiguration configuration)
        {
        ...
        public async Task<IActionResult> SearchStudent(StudentModel StudentData)
        {
            ...
            using (var client = clientFactory.CreateClient("BaseClient"))
            {
                try
                {
                    ...
                    var response = await client.PostAsync("Student/SearchStudent", httpContent);
                    if (response.IsSuccessStatusCode)
                    {
                        //Install-Package Microsoft.AspNet.WebApi.Client
